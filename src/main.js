...
import { getProjectsFromFile, getProjectsByQuerystring } from "@/projects";
import { getStandaloneParams } from "@/standalone";
import { getParameterByName, getTopItem, getTopTagName } from "@/utils";
    ...
    this.loadConfig().then(() => {
      if (this.standalone) {
        getStandaloneParams(window.location.origin).then(params => {
          this.gitlab = params.gitlab || this.gitlab;
          this.token = params.token;
          this.ref = params.ref;
          this.projectsFile = "standalone";
          this.projects = params.projects;
          this.gitlabciProtocol =
            params.gitlabciProtocol || DEFAULT_GITLABCI_PROTOCOL;
          this.hideSuccessCards =
            params.hideSuccessCards || DEFAULT_HIDE_VERSION;
  ...
  methods: {
    loadConfig() {
      ...
      return getConfigFromFile(this.configFile)
        .then(({ config, projects }) => {
          ...
          this.gitlab = config.gitlab || this.gitlab;
          this.token = config.token;
          this.ref = config.ref;
          this.projects = projects;
          this.projectsFile = config.projectsFile;
          this.gitlabciProtocol =
            config.gitlabciProtocol || DEFAULT_GITLABCI_PROTOCOL;
        ...
        .catch(e => {
          ...
          this.gitlab = getParameterByName("gitlab") || this.gitlab;
          ...
          this.gitlabciProtocol =
            getParameterByName("gitlabciProtocol") || DEFAULT_GITLABCI_PROTOCOL;
          this.hideSuccessCards = getParameterByName("hideSuccessCards");
          this.apiVersion =
            getParameterByName("apiVersion") || DEFAULT_API_VERSION;
          if (this.hideSuccessCards == null) {
            this.hideSuccessCards = DEFAULT_HIDE_SUCCESS_CARDS;
          }
    ...
    loadRepositories(repos) {
      ...
      for (const index in repos) {
        try {
          ...
          const { namespace, project, branch, description } = repository;
    ...
    startup() {
      ...
      if (this.standalone || Array.isArray(this.projects)) {
        ...
        getProjectsFromFile(this.projectsFile).then(projects =>
    ...
    handlerError(error) {
      ...
      if (error.message === "Wrong format") {
        this.onError = {
          code: 2,
          message:
            "Wrong projects format! Try: 'namespace/project' or 'namespace/project/branch'",
        };
      }
      ...
      if (error.message === "404 - Not Found.") {
        ...
        this.onError = {
          code: 5,
          message: `Project(s) not found: ${this.wrongProjects.map(
            w => " " + w
          )}`,
        };
      }
    ...
    configValid() {
      const { projectsFile, token, gitlab, projectsParam, configFile } = this;
      if (configFile) {
        return true;
      }
      if (
        (projectsParam == null && projectsFile == null) ||
        token == null ||
        gitlab == null
      ) {
        return false;
    ...
    setupDefaults(provider) {
      const { gitlab, token, gitlabciProtocol, apiVersion } = this;
        ...
        getProjects(repo.nameWithNamespace)
          .then(response => {
            ...
            if (this.apiVersion === DEFAULT_API_VERSION) {
              this.fetchBuilds({ repo, project: response.data }).then(
                this.loadBuilds.bind(this)
              );
            } else {
    ...
    getLinkToBranch(project, repo) {
      return `${this.gitlabciProtocol}://${this
        .gitlab}/${project.path_with_namespace}/tree/${repo.branch}`;
    },
    getLinkToPipeline(project, repo, buildId) {
      return `${this.gitlabciProtocol}://${this
        .gitlab}/${project.path_with_namespace}/pipelines/${buildId}`;
    },
    getLinkToJob(project, repo, buildId) {
      return `${this.gitlabciProtocol}://${this
        .gitlab}/${project.path_with_namespace}/-/jobs/${buildId}`;
    },
    loadBuilds(onBuilds, data, repo, project, tag) {
      ...
      for (const index in onBuilds) {
        ...
        if (b.project === repo.projectName && b.branch === repo.branch) {
          updated = true;
    ...
    fetchPipelines(selectedProject) {
      ...
      const { repo, project } = selectedProject;
      getCommits(project.id, repo.branch)
        .then(({ data }) => {
                ...
                .then(pipeline => {
                  ...
                  this.onBuilds.forEach(build => {
                    if (
                      ...
                      build.started_at = moment(
                        lastPipeline.started_at
                      ).fromNow();
                      ...
                      build.link_to_branch = this.getLinkToBranch(
                        project,
                        repo
                      );
                      build.link_to_build = this.getLinkToPipeline(
                        project,
                        repo,
                        lastPipeline.id
                      );
                  ...
                  if (!updated) {
                    ...
                    buildToAdd.link_to_branch = this.getLinkToBranch(
                      project,
                      repo
                    );
                    buildToAdd.link_to_build = this.getLinkToPipeline(
                      project,
                      repo,
                      lastPipeline.id
                    );
    ...
    handlerBranch(onBuilds, repo, project, lastCommit) {
      return getBuilds(project.id, lastCommit)
        .then(response => {
          ...
          this.handlerBuilds(onBuilds, builds, repo, project).then(tag => {
    ...
    fetchBuilds(selectedProjects) {
      ...
      const { repo, project } = selectedProjects;
